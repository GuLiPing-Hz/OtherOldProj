...
CStream::CStream( ULONG ulInitialSize, DWORD dwBaseAllocSize, IUnknown *pOuterUnk ) 
	: m_cRef(0), m_pbMem(0), m_ullEnd(0), m_ullPos(0), m_ullSize(0), m_iNumCommittedPages(0), m_pOuterUnk(pOuterUnk),
		m_uNumReservedPages(0), BASE_ALLOC_SIZE(dwBaseAllocSize),m_bQuit(false)
{
    ...
    ULONG *pcbRead )
{
	...
	if(cb + m_ullPos > m_ullEnd){
		if(m_ullEnd < m_ullSize){
			WaitandResetReadEvent();
			if(m_bQuit) return S_OK;
		}
...
STDMETHODIMP CStream::WriteEx(const void *pv,ULONG cb,ULONG *pcbWritten){
	if( !pv )
		return STG_E_INVALIDPOINTER;

	// expand committed memory if necessary
	if( cb + m_ullEnd > m_ullSize )
	{
		// dertermine the size needed 
		ULARGE_INTEGER uli; 
		uli.QuadPart = m_ullEnd + cb;
		RETURN_IF_FAILED( SetSize( uli ) );
	}

	// write the data
	CopyMemory( &m_pbMem[m_ullEnd], pv, cb );
