...
/*HGE *hgeGUI::cls_sceneMgr=0;*/
/*HGE *hgeGUIObject::cls_sceneMgr=0;*/
COpenGLWindow* hgeGUI::cls_gl = NULL;

hgeGUI::hgeGUI()
{
	/*cls_sceneMgr=hgeCreate(HGE_VERSION);*/
...
void hgeGUI::SetCursor(CGLSprite *spr)
{
	sprCursor=spr;
}
...
void hgeGUI::Render()
{
	...
	if(cls_gl->getSceneMgr()->Input_IsMouseOver() && sprCursor) sprCursor->drawRender(mx,my);
}
...
// Update the mouse variables
	...
	cls_gl->getSceneMgr()->Input_GetMousePos(&mx, &my);
	bLPressed  = cls_gl->getSceneMgr()->Input_KeyDown(HGEK_LBUTTON);
	bLReleased = cls_gl->getSceneMgr()->Input_KeyUp(HGEK_LBUTTON);
	bRPressed  = cls_gl->getSceneMgr()->Input_KeyDown(HGEK_RBUTTON);
	bRReleased = cls_gl->getSceneMgr()->Input_KeyUp(HGEK_RBUTTON);
	nWheel=cls_gl->getSceneMgr()->Input_GetMouseWheel();
...
// Handle keys	
	...
	key=cls_gl->getSceneMgr()->Input_GetKey();
	...
	{
		if(ctrlFocus->KeyClick(key, cls_gl->getSceneMgr()->Input_GetChar())) return ctrlFocus->id;
	}

// Handle mouse

	bool bLDown = cls_gl->getSceneMgr()->Input_GetKeyState(HGEK_LBUTTON);
	bool bRDown = cls_gl->getSceneMgr()->Input_GetKeyState(HGEK_RBUTTON);
