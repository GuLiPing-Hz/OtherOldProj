...
ImgsetMgr *hgeStringTable::cls_imgsetMgr = NULL;
...
hgeStringTable::hgeStringTable(const char *filename)
{
	...
	data=cls_imgsetMgr->Resource_Load(filename, &size);
	if(!data) return;

	desc = new char[size+1];
	memcpy(desc,data,size);
	desc[size]=0;
	cls_imgsetMgr->Resource_Free(data);

	// check header
	if(memcmp(desc, STRHEADERTAG, sizeof(STRHEADERTAG)-1))
	{
		//w riteLog(STRFORMATERROR, filename);
		LOG_ERN2(STRFORMATERROR,filename);
		delete[] desc;	
		return;
	}
	...
	for(;;)
	{
		...
		// skip whitespaces to '='
		while(isspace(*pdesc)) pdesc++;
		if(*pdesc!='=')	{ LOG_ERN2(STRFORMATERROR,filename);/*w riteLog(STRFORMATERROR, filename);*/ break; }
		pdesc++;
