...
ImgsetMgr* RScriptParser::cls_imgsetMgr = NULL;
...
{
// 	w riteLog("[DXLOG RScriptParser::ScriptPostError]:%s, line %d: %s'%s'%s",get_name(), get_line(), msg1, tokenvalue[0] ? tkn_string():"<EOF>", msg2);
	char buf[260];
	sprintf_s(buf,259,"%s'%s'%s",msg1, tokenvalue[0] ? tkn_string():"<EOF>", msg2);
	LOG_ERN0(buf);
}
