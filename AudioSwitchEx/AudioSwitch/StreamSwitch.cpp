...
// 	}
    ...
    if (!IsActive()) 
	{
// 		sprintf(buf,"InActive rtStart %lld,rtStop %lld,cbBuffer %d\n",rt_start,rt_stop,cbBuffer);
...
// 		}
		...
		while(m_bRunning&&rt_start>((static_cast<CStreamSwitcherFilter*>(m_pFilter))->m_rtStart))
		{
			Sleep(1);
			if (IsActive())
			{
	...
	{
// 		sprintf(buf," Active rtStart %lld,rtStop %lld,cbBuffer %d\n",rt_start,rt_stop,cbBuffer);
// 		OutputDebugStringA(buf);
	...
	HRESULT hr1 = (static_cast<CStreamSwitcherFilter*>(m_pFilter))->ChangePitch(pOutSample);
	if (FAILED(hr1))
	{
		OutputDebugStringA("change pitch failed but nothing,may be the audio media type is not wave");
	}
    if (S_OK == hr) 
	{
...
void CStreamSwitcherFilter::UpdateCurrentTime(const REFERENCE_TIME rtStart,const REFERENCE_TIME rtStop)
{
	m_rtStart = rtStart;
	m_rtStop = rtStop;
	m_rtSampleDelta = m_rtStop - m_rtStart;
}
...
HRESULT CStreamSwitcherFilter::Transform(IMediaSample* pIn, IMediaSample* pOut)
{
	...
	if (mt.formattype == FORMAT_WaveFormatEx && mt.cbFormat >= sizeof(WAVEFORMATEX))//判断是否是WAVEFORMATEX
	{
		...
		case MonoL:
			{
				...
				if (m_bCopySTrack)
				{
					...
					while (tmpCopied < totalCopy)
					{
						...
						memcpy(pDataOut+tmpLenOut,pDataIn+tmpLenIn,wBytesPerSample);//左声道拷贝右声道数据
						tmpLenOut += wBytesPerSample;
				...
				else
				{
					...
					while (tmpCopied < totalCopy)
					{
						...
						{
							memcpy(pDataOut+tmpLenOut,&nChar,wBytesPerSample);//右声道置为0
						}
						else if (wBytesPerSample == 2)
						{
							memcpy(pDataOut+tmpLenOut,&nShort,wBytesPerSample);//右声道置为0
						}
						tmpLenOut += wBytesPerSample;
						tmpLenIn += wBytesPerSample*2;
						tmpCopied ++;
		...
		case MonoR:
			{
				...
				if (m_bCopySTrack)
				{
					...
					while (tmpCopied < totalCopy)
					{
						...
						tmpLenOut += wBytesPerSample;
						memcpy(pDataOut+tmpLenOut,pDataIn+tmpLenIn,wBytesPerSample);//右声道拷贝右到声道
						tmpLenOut += wBytesPerSample;
						tmpLenIn += wBytesPerSample*2;
						tmpCopied ++;
					}
				...
				{
// 					CMediaType nmt =mt ;
// 					WAVEFORMATEX* pwfm = reinterpret_cast<WAVEFORMATEX*>(nmt.Format());
// 					pwfm->nChannels = 1;
// 					pwfm->nBlockAlign = pwfm->nChannels * pwfm->wBitsPerSample/8;
// 					pwfm->nAvgBytesPerSec = pwfm->nBlockAlign*pwfm->nSamplesPerSec/2;
// 
// 					if(!ChangeNextPinMT(nmt))
// 					{
// 						goto rejectmt;
// 					}
					long tmpCopied = 0;
...
// 	{
// 		//m_bBeginFlush --;
// 		OutputDebugString(__TEXT("Return S_OK"));
// 		return S_OK;
// 	}
	//OutputDebugString(__TEXT("Real Deliver\n"));
...
{
	char buf[260] = {0};
	if (tStart != m_rtNewStart+m_rtSampleDelta)
	{
		int nGLP = 0;
	}
	static int nGLP;
	sprintf(buf,"DeliverNewSegment tStart %lld,tStop %lld,dRate %f\n***********************************************************%d\n"
