...
HRESULT CMpegSplitterFile::Init(IAsyncReader* pAsyncReader)
{
    ...
    if (IsRandomAccess() || IsStreaming()) {
        ...
        __int64 pfp = 0;
        const int k = 5;
		//SearchStreams(0,GetLength(),pAsyncReader);
// 		TRACE("******************************************glp*******************\n");
        for (int i = 0; i <= k; i++) {
...
REFERENCE_TIME CMpegSplitterFile::NextDTS(DWORD TrackNum,bool bReback,__int64* prePos)
{
	REFERENCE_TIME rt = -1;
	__int64 rtpos = -1;
	...
	while (GetRemaining()) 
	{
		if (m_type == mpeg_ps || m_type == mpeg_es) 
		{
			...
			if ((b >= 0xbd && b < 0xf0) || (b == 0xfd)) //audio video 
			{
				peshdr h;
				if (!Read(h, b) || !h.len) {
					continue;
				...
				if (h.bdts && AddStream(0, b, h.id_ext, h.len) == TrackNum) {
		...
		else if (m_type == mpeg_ts) 
		{
			...
			{
				peshdr h2;
				if (NextMpegStartCode(b, 4) && Read(h2, b)) 
				{ // pes packet
					if (h2.bdts && AddStream(h.pid, b, 0, DWORD(h.bytes - (GetPos() - rtpos)),true) == TrackNum)
					{
...
REFERENCE_TIME CMpegSplitterFile::NextPTS(DWORD TrackNum,bool bReback,__int64* prePos)
{
    REFERENCE_TIME rt = -1;
    __int64 rtpos = -1;
    ...
    while (GetRemaining()) 
	{
        if (m_type == mpeg_ps || m_type == mpeg_es) 
		{
            ...
            if ((b >= 0xbd && b < 0xf0) || (b == 0xfd)) //audio video 
			{
                ...
                if (h.bpts && AddStream(0, b, h.id_ext, h.len) == TrackNum) {
                    //ASSERT(h.pts >= m_rtMin && h.pts <= m_rtMax);
                    rt = h.pts;
					Seek(pos + h.len);
                    break;
		...
		else if (m_type == mpeg_ts) 
		{
            ...
            if (h.payload && h.payloadstart && ISVALIDPID(h.pid))
			{
                ...
                if (NextMpegStartCode(b, 4) && Read(h2, b)) 
				{ // pes packet
                    if (h2.bpts && AddStream(h.pid, b, 0, DWORD(h.bytes - (GetPos() - rtpos)),true) == TrackNum)
					{
                        //ASSERT(h2.pts >= m_rtMin && h2.pts <= m_rtMax);
                        rt = h2.pts;
						Seek(h.next);
    ...
    while (GetPos() < stop) {
        ...
        if (m_type == mpeg_ps || m_type == mpeg_es) {
            ...
            if (b == 0xba) { // program stream header
				...
				//TRACE("ID:%x , PTS REFTIME: %lld,DTS REFTIME %lld , len: %d",b,h.pts,h.dts,h.len);
                if (h.bpts)
				{
                    if (m_rtMin == _I64_MAX)
					{
                        m_rtMin = h.pts;
                        m_posMin = GetPos();
                        //TRACE(_T("m_rtMin(SearchStreams)=%s£¬%lld\n"), ReftimeToString(m_rtMin),m_rtMin);
                    }
            ...
            if (h.payload && ISVALIDPID(h.pid)) {
                ...
                if (h.payloadstart && NextMpegStartCode(b, 4) && Read(h2, b)) { // pes packet
                    ...
                    if (h2.bpts && CalcDuration && (m_AlternativeDuration || (GetMasterStream() && GetMasterStream()->GetHead() == h.pid))) {
                ...
                if (!CalcDuration) {
                    AddStream(h.pid, b, 0, DWORD(h.bytes - (GetPos() - pos)),true);
                }
            }
...
DWORD CMpegSplitterFile::AddStream(WORD pid, BYTE pesid, BYTE ps1id, DWORD len,bool bTs)
{
    if (pid) {
        if (pesid) {
            m_pid2pes[pid] = pesid;
