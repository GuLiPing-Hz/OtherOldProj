...
const AMOVIESETUP_FILTER sudFilter[] = {
    ...
    {&__uuidof(CMpegSourceFilter), MpegSourceName, MERIT_UNLIKELY, 0, NULL},
};
...
//     }
    ...
    for (int i = 0; i < _countof(m_pFile->m_streams); i++)
	{
        ...
        while (pos) 
		{
            ...
            CBaseSplitterOutputPin* pPinOut = DEBUG_NEW CMpegSplitterOutputPin(mts, str, this, &hr, m_pFile->m_type,(DWORD)s,&s.mt);
            IncrementPinVersion();
			if (i == CMpegSplitterFile::subpic) 
			{
                (static_cast<CMpegSplitterOutputPin*>(pPinOut))->SetMaxShift(_I64_MAX);
            }
...
bool CMpegSplitterFilter::DemuxInit()
{
	...
	m_rtStartOffset = m_rtStart ? m_rtStartOffset : 0;

    return true;
}

void CMpegSplitterFilter::DemuxBeginFlush()
{
	//m_rtStart -=  5000000;

	CBaseSplitterFilter::DemuxBeginFlush();
}

void CMpegSplitterFilter::DemuxOnlySeek(REFERENCE_TIME rt,CBaseSplitterOutputPin* pOut,__int64* pInt64)
{
	...
	pOut->m_bSeeked = true;
// 	char buf[260] = {0};
// 	sprintf_s(buf,259,"DemuxOnlySeek Pos : %lld;",*pInt64);
// 	OutputDebugStringA(buf);
}

void CMpegSplitterFilter::DemuxSeek(REFERENCE_TIME rt,CBaseSplitterOutputPin* pOut,__int64* pInt64)
{
	CAutoLock lock(&m_csReadFile);
    ...
    if (m_pFile->IsStreaming()) 
	{
		...
		if (pInt64)
		{
			*pInt64 = max(0, m_pFile->GetLength() - 100 * 1024);
		}
        m_rtStartOffset = m_pFile->m_rtMin + m_pFile->NextPTS(pMasterStream->GetHead());
        return;
    }

    const REFERENCE_TIME rtPreroll = 10000000;
	const REFERENCE_TIME rtPreroll2 = 5000000;
	...
	else
	{
		...
		__int64 tmpPos = seekpos;
        __int64 minseekpos = _I64_MAX;

        REFERENCE_TIME rtmax = rt - rtPreroll;
        REFERENCE_TIME rtmin = rt + rtPreroll;

//         if (false/*m_rtStartOffset == 0*/) 
// 		{
...
// 					DWORD TrackNum = pOut->m_nTrackNum;
...
// 							{
// 								TRACE("TrackNum %d,rtmin : %lld ,rt2 :%lld(rt :%lld),rtmax : %lld",pOut->m_nTrackNum,rtmin,rt2,rt,rtmax);
//                                 minseekpos = min(minseekpos, curpos);
//                                 break;
//                             }
// 
//                             curpos -= (__int64)(1.0 * dt / m_rtDuration * len);//继续往前寻找
//                             m_pFile->Seek(curpos);
//                             //pdt = dt;
//                         }
//                     }
...
// 		else 
		{
            ...
            REFERENCE_TIME rt2 = rt - rtPreroll;
			rt2 = rt2 < 0 ? 0 : rt2;
            seekpos = (__int64)(1.0 * rt2 / m_rtDuration * len);
            m_pFile->Seek(seekpos);
			DWORD TrackNum = pMasterStream->GetHead();
			//DWORD TrackNum = pMasterStream->GetTail();
			//DWORD TrackNum = pOut->m_nTrackNum;
			REFERENCE_TIME rtNextPts;
			REFERENCE_TIME rtDuration = rt + 5000000;
			rtDuration = rtDuration> m_rtDuration ? m_rtDuration : rtDuration;
			do 
			{
				rtNextPts = m_pFile->NextPTS(TrackNum,false);
				tmpPos = m_pFile->GetPos();
				//m_rtStartOffset = rtNextPts!=-1 ? m_pFile->m_rtMin + rtNextPts - rt :m_pFile->m_rtMax;
				m_rtStartOffset = rtNextPts!=-1 ? rtNextPts : m_pFile->m_rtMax;
			} while (m_rtStartOffset < rtDuration);

			REFERENCE_TIME rtTmp = 0;
			REFERENCE_TIME rtTmp2 = 0;
			if (pOut->m_eStreamType != video/*TrackNum != pOut->m_nTrackNum*/)
			{
				rt2 -= 5000000;
				rt2 = rt2 < 0 ? 0 : rt2;
				tmpPos = (__int64)(1.0*rt2/m_rtDuration *len);
				__int64 tmpPos2 = tmpPos;
				m_pFile->Seek(tmpPos);
				rtDuration = rtNextPts /*+ 10000000*/;
				rtDuration = rtDuration> m_rtDuration ? m_rtDuration : rtDuration;
				do
				{
					tmpPos2 = tmpPos;
					rtTmp2 = rtTmp;
					rtTmp = m_pFile->NextPTS(pOut->m_nTrackNum,false,&tmpPos) ;
					rtTmp = rtTmp!=-1 ? rtTmp : m_pFile->m_rtMax;
				}while(rtTmp < rtDuration);
// 				pOut->m_bNeedCut = true;
// 				pOut->m_fCutPercent = 1.0f*(rtTmp-rtNextPts)/(rtTmp-rtTmp2);
				tmpPos = tmpPos2;
			}
// 			else
// 			{
...
// 				do 
// 				{
// 					rtTmp = m_pFile->NextPTS(TrackNum,false,&tmpPos);
// 				} while (rtTmp < rt);
// 			}

			TRACE("TrackNum %d,m_rtStartOffset : %lld ,rtTmp:%lld,rtNextPts:%lld(rt :%lld),Pos: %lld\n",pOut->m_nTrackNum,m_rtStartOffset,rtTmp,rtNextPts,rt,tmpPos);
        }
...
HRESULT CMpegSplitterFilter::DemuxNextPacket(REFERENCE_TIME rtStartOffset,CBaseSplitterOutputPin* pOut,__int64* pInt64,Packet* pPacket)
{
	CAutoLock lock(&m_csReadFile);
	...
	if (m_pFile->m_type == mpeg_ps || m_pFile->m_type == mpeg_es) 
	{
		while(TRUE)
		{
			...
			{ // pes packet
				...
				if (pOut->m_nTrackNum != TrackNumber)
				{
					if (pInt64)
					{
						*pInt64 = pos+h.len;
				...
				//if (GetOutputPin(TrackNumber)) 
				{
					...
					pPacket->TrackNumber = TrackNumber;
					pPacket->bSyncPoint = !!h.bpts;
					pPacket->bAppendable = !h.bpts;
					pPacket->rtStart = h.bpts ? (h.pts - rtStartOffset) : Packet::INVALID_TIME;
					pPacket->rtStop = pPacket->rtStart + 1;
					WORD tmp_nLen = h.len - (size_t)(m_pFile->GetPos() - pos);
					pPacket->SetCount(tmp_nLen);
					m_pFile->ByteRead(pPacket->GetData(), tmp_nLen);

					hr = DeliverPacket(pPacket,rtStartOffset,pOut);
// 					if (pOut->m_nTrackNum != 57344)
// 					{
// 						char buf[260] = {0};
// 						sprintf_s(buf,259,"Current TrackNum:%d,rtStart:%lld,rtStop:%lld\n",pOut->m_nTrackNum,pPacket->rtStart,pPacket->rtStop);
// 						OutputDebugStringA(buf);
// 					}
				}
				if (pInt64)
	...
	else if (m_pFile->m_type == mpeg_ts) 
	{
		...
		while(TRUE)
		{
			...
			if (h.payload && ISVALIDPID(h.pid)) 
			{
				...
				if (h.payloadstart && m_pFile->NextMpegStartCode(b, 4) && m_pFile->Read(h2, b)) 
				{ // pes packet
					...
					TrackNumber = m_pFile->AddStream(h.pid, b, 0, h.bytes - (DWORD)(m_pFile->GetPos() - pos),true);
				}
				...
				//if (GetOutputPin(TrackNumber))
				{
					...
					pPacket->TrackNumber = TrackNumber;
					pPacket->bSyncPoint = !!h2.bpts;
					pPacket->bAppendable = !h2.bpts;
					...
					pPacket->rtStart = h2.bpts ? (h2.pts - rtStartOffset) : Packet::INVALID_TIME;
					pPacket->rtStop = pPacket->rtStart + 1;
					pPacket->SetCount(h.bytes - (size_t)(m_pFile->GetPos() - pos));

					int nBytes = int(h.bytes - (m_pFile->GetPos() - pos));
					m_pFile->ByteRead(pPacket->GetData(), nBytes);
	...
	else if (m_pFile->m_type == mpeg_pva) 
	{
		...
		while(TRUE)
		{
			...
			//if (GetOutputPin(TrackNumber)) 
			{
				...
				pPacket->TrackNumber = TrackNumber;
				pPacket->bSyncPoint = !!h.fpts;
				pPacket->bAppendable = !h.fpts;
				pPacket->rtStart = h.fpts ? (h.pts - rtStartOffset) : Packet::INVALID_TIME;
				pPacket->rtStop = pPacket->rtStart + 1;
				pPacket->SetCount(h.length);

				m_pFile->ByteRead(pPacket->GetData(), h.length);
				hr = DeliverPacket(pPacket,rtStartOffset,pOut);
			}
...
bool CMpegSplitterFilter::isEof()
{
	if(m_rtCurrent >= m_rtStop)
		m_bEof = true;
	return m_bEof ?m_bEof:m_bEof=FAILED(m_pFile->HasMoreData(1024 * 500));
}

REFERENCE_TIME CMpegSplitterFilter::getStartOffset(CBaseSplitterOutputPin* pOut)
{
	REFERENCE_TIME rtStartOffset = m_rtStartOffset ? m_rtStartOffset : (m_rtStartOffset = m_pFile->m_rtMin);

	return rtStartOffset;
}
...
CMpegSplitterOutputPin::CMpegSplitterOutputPin(CAtlArray<CMediaType>& mts, LPCWSTR pName, CSource* pFilter, HRESULT* phr, int type ,DWORD nTrackNum,CMediaType* pMT)
...
HRESULT CMpegSplitterOutputPin::dealPacket(Packet* p)
{
    CAutoLock cAutoLock(this);
...
//             }
    ...
    if (m_mt.subtype == MEDIASUBTYPE_AAC) { // special code for aac, the currently available decoders only like whole frame samples
        ...
        while (m_p && m_p->GetCount() > 9) {
            ...
            HRESULT hr = __super::dealPacket(p2);
            if (hr != S_OK) {
                return hr;
            }
        }
        ...
        for (POSITION pos = m_pl.GetHeadPosition(); pos; m_pl.GetNext(pos)) {
            ...
            if ((pData[4] & 0x1f) == 0x09 || (!m_fHasAccessUnitDelimiters && pPacket->rtStart != Packet::INVALID_TIME)) {
                ...
                HRESULT hr = __super::dealPacket(p);
                if (hr != S_OK) {
                    return hr;
                }
