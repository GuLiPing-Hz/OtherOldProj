 ...
 * MERCHANTAB1462ILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
...
#include "thirdparty/Bento4/Core/Ap4StssAtom.h"
#include "thirdparty/Bento4/Core/Ap4StsdAtom.h"
#include "thirdparty/Bento4/Core/Ap4IsmaCryp.h"
#include "thirdparty/Bento4/Core/Ap4AvcCAtom.h"
#include "thirdparty/Bento4/Core/Ap4ChplAtom.h"
#include "thirdparty/Bento4/Core/Ap4FtabAtom.h"
#include "thirdparty/Bento4/Core/Ap4DataAtom.h"
#include "thirdparty/Bento4/Core/Ap4PaspAtom.h"
...
const AMOVIESETUP_FILTER sudFilter[] = {
    {&__uuidof(CMP4SplitterFilter), MP4SplitterName, MERIT_NORMAL, _countof(sudpPins), sudpPins},
    {&__uuidof(CMP4SourceFilter), MP4SourceName, MERIT_NORMAL, 0, NULL},
    {&__uuidof(CMPEG4VideoSplitterFilter), MPEG4SplitterName , MERIT_NORMAL, _countof(sudpPins), sudpPins},
    {&__uuidof(CMPEG4VideoSourceFilter), MPEG4SourceName ,MERIT_NORMAL, 0, NULL},
};
...
HRESULT CMP4SplitterFilter::CreateOutputs(IAsyncReader* pAsyncReader)
{
                ...
                item = item->GetNext()) 
		{
            ...
            if (AP4_SampleDescription* desc = track->GetSampleDescription(sample.GetDescriptionIndex())) {
                ...
                if (AP4_AvcCAtom* avcC = dynamic_cast<AP4_AvcCAtom*>(avc1->GetChild(AP4_ATOM_TYPE_AVCC))) {
                    ...
                    int lnko = LNKO(aspect.cx, aspect.cy);//最大公约数
                    if (lnko > 1) {
            ...
            CBaseSplitterOutputPin* pPinOut = DEBUG_NEW CMP4SplitterOutputPin(mts, name, this, &hr,id,&mt);
			IncrementPinVersion();
            if (!TrackName.IsEmpty()) {
            ...
            if (mts.GetCount() == 1 && mts[0].subtype == MEDIASUBTYPE_ASS2) {
                ...
                CBaseSplitterOutputPin* pPinOut = DEBUG_NEW CMP4SplitterOutputPin(mts, name + postfix, this, &hr,track->GetId(),&mt);
    ...
    //TRACE(_T("CMP4SplitterFilter m_pOutputs.GetCount()  = %d") , CSource::GetPinCount());

	return CSource::GetPinCount() > 0 ? S_OK : E_FAIL;
}
...
bool CMP4SplitterFilter::DemuxInit()
{
	...
	if (!track)
	{
		OutputDebugStringA("there is no video in the file\n");
		track = movie->GetTrack(AP4_Track::TYPE_AUDIO);
		if (!track)
		{
			OutputDebugStringA("there is no audio in the file too\n");
			return false;
		}
	}
	...
	if (!m_rtStart)
	{
		m_mapStartOffset.clear();
		for(int i=0;i<CSource::GetPinCount();i++)
		{
			CBaseSplitterOutputPin* pOut = static_cast<CBaseSplitterOutputPin*>(CSource::GetPin(i));
			AP4_Sample sample;
			if (AP4_SUCCEEDED(track->GetSample(0, sample)) && (pOut->m_eStreamType == video)) 
			{
				REFERENCE_TIME rt = (REFERENCE_TIME)(10000000.0 / track->GetMediaTimeScale()*sample.GetCts());
				m_rtStartOffset = rt;
				m_mapStartOffset.insert(std::make_pair(pOut->m_nTrackNum,rt));
			}
			else
			{
				m_mapStartOffset.insert(std::make_pair(pOut->m_nTrackNum,0LL));
			}
		}
	}
...
REFERENCE_TIME CMP4SplitterFilter::getStartOffset(CBaseSplitterOutputPin* pOut)
{
	return m_mapStartOffset[pOut->m_nTrackNum];
}

bool CMP4SplitterFilter::isEof()
{
...
HRESULT CMP4SplitterFilter::DemuxNextPacket(REFERENCE_TIME rtStartOffset,CBaseSplitterOutputPin* pOut,__int64* pInt64,Packet* pPacket)
{
	CAutoLock lock(&m_csReadFile);
...
// 		//该map保存相应id的track pos 数据，例如如果有两路音轨，则有2个id，
//         POSITION pos = m_trackpos.GetStartPosition();
//         while (pos) 
// 		{
//             CAtlMap<DWORD, trackpos>::CPair* pPair = m_trackpos.GetNext(pos);
...
//         }
		...
		AP4_Track* track = movie->GetTrack(pOut->m_nTrackNum);
		if (*pInt64 >= track->GetSampleCount())
		{
			return S_FALSE;
		}
        ...
        AP4_DataBuffer data;
		//尝试读取一个sample
        while ( AP4_SUCCEEDED(track->ReadSample(/*pPairNext->m_value.index*/*pInt64, sample, data))) 
		{
            pPacket->TrackNumber = (DWORD)track->GetId();
			
			if (pPacket->TrackNumber != pOut->m_nTrackNum)
			{
				ASSERT( 0 );
				++ (*pInt64);
				continue;
			...
			const CMediaType& mt = pOut->CurrentMediaType();
			//百纳秒/时间单位*时间
            pPacket->rtStart = (REFERENCE_TIME)(10000000.0 / track->GetMediaTimeScale() * sample.GetCts())-rtStartOffset;
            pPacket->rtStop = pPacket->rtStart + (REFERENCE_TIME)(10000000.0 / track->GetMediaTimeScale() * sample.GetDuration());
            pPacket->bSyncPoint = TRUE;
            ...
            if (AP4_StssAtom* stss = dynamic_cast<AP4_StssAtom*>(track->GetTrakAtom()->FindChild("mdia/minf/stbl/stss"))) 
			{
                if (stss->m_Entries.ItemCount() > 0) 
				{
					...
					{
                        if (stss->m_Entries[i] - 1 < /*pPairNext->m_value.index*/*pInt64) 
						{
                            continue;
                        }
                        if (stss->m_Entries[i] - 1 == /*pPairNext->m_value.index*/*pInt64) 
            ...
            if (track->GetType() == AP4_Track::TYPE_AUDIO && data.GetDataSize() >= 1 && data.GetDataSize() <= 16) 
			{
                ...
                int nBlockAlign;//限定blockalign 来以此读取track sample
                if (wfe->nBlockAlign == 0) 
				{
                    nBlockAlign = 1200;
                } 
				else if (wfe->nBlockAlign <= 16) 
				{ // for PCM (from 8bit mono to 64bit stereo), A-Law, u-Law
                    nBlockAlign = wfe->nBlockAlign * (wfe->nSamplesPerSec >> 7); // 1/16s=62.5ms
                } 
				else 
				{
                    nBlockAlign = wfe->nBlockAlign;
                    /*pPairNext->m_value.index*/*pInt64 -= /*pPairNext->m_value.index*/(*pInt64) % wfe->nBlockAlign;//调整
                }

                pPacket->rtStop = pPacket->rtStart;
                //TRACE(_T("track->GetSampleCount() %d %d "), track->GetSampleCount(), /*pPairNext->m_value.index*/*pInt64);
                bool bFirst = true;
				//读入到packet中
                while (AP4_SUCCEEDED(track->ReadSample(/*pPairNext->m_value.index*/*pInt64, sample, data))) 
				{
                    ...
                    if (bFirst) 
					{//第一次，则设置数据长度，起始时间
                        pPacket->SetData(ptr, size);
                        pPacket->rtStart = pPacket->rtStop = (REFERENCE_TIME)(10000000.0 / track->GetMediaTimeScale() * sample.GetCts());
                        bFirst = false;
                    } 
					else 
					{//此后，只需要加入数据即可
					...
					//根据新加入的sample的时长，调整结束时间
                    pPacket->rtStop += (REFERENCE_TIME)(10000000.0 / track->GetMediaTimeScale() * sample.GetDuration());
					//如果已经超过track的samplecount 或者 packet超过blockalign
                    if (/*pPairNext->m_value.index*/*pInt64 + 1 >= track->GetSampleCount() || (int)pPacket->GetCount() >= nBlockAlign) 
					{
                        break;
                    }
					//index递增
			...
			{
				static CStringA dlgln_bkg, dlgln_plaintext;
                ...
                if (avail > 2) {
                    ...
                    if (size <= avail - 2) 
					{
                        ...
                        if (mt.subtype == MEDIASUBTYPE_ASS2)
						{
                            ...
                            if (AP4_UnknownSampleDescription* unknown_desc = dynamic_cast<AP4_UnknownSampleDescription*>(desc)) 
							{ // TEMP
                                AP4_SampleEntry* sample_entry = unknown_desc->GetSampleEntry();
								...
								else if (AP4_Tx3gSampleEntry* tx3g = dynamic_cast<AP4_Tx3gSampleEntry*>(sample_entry)) 
								{
                                    ...
                                    CStringW ssa = ConvertTX3GToSSA(
                                                       ...
                                                       (pPacket->rtStop - pPacket->rtStart) / 10000,
                                                       rbox);
                                    dlgln = UTF16To8(ssa);
                        ...
                        pPacket->SetData((LPCSTR)dlgln, dlgln.GetLength());
                    }
                }
                ...
                if (!dlgln_bkg.IsEmpty()) 
				{
                    ...
                    p2->TrackNumber = pPacket->TrackNumber;
                    p2->rtStart = pPacket->rtStart;
                    p2->rtStop = pPacket->rtStop;
                    p2->bSyncPoint = pPacket->bSyncPoint;
				...
				{
                    CAutoPtr<Packet> p2(DEBUG_NEW Packet());
                    p2->TrackNumber = pPacket->TrackNumber ^ 0x80402010;
                    p2->rtStart = pPacket->rtStart;
                    p2->rtStop = pPacket->rtStop;
                    p2->bSyncPoint = pPacket->bSyncPoint;
			...
			{
                pPacket->SetData(data.GetData(), data.GetDataSize());
            }
            DeliverPacket(pPacket,rtStartOffset,pOut);
			break;
        }

        AP4_Sample sample2;
        if (AP4_SUCCEEDED(track->GetSample(++/*pPairNext->m_value.index*/(*pInt64), sample2))) 
		{
            //pPairNext->m_value.ts = sample2.GetCts();
        }
    }
...
void CMP4SplitterFilter::DemuxOnlySeek(REFERENCE_TIME rt,CBaseSplitterOutputPin* pOut,__int64* pInt64)
{
	CAutoLock lock(&m_csReadFile);

	if (m_listAudioOutput.size() < 2)
	{
		return ;
	}
	if (m_listAudioOutput.front() == pOut)
	{
		*pInt64 = m_listAudioOutput.back()->m_prepos[0];
	}
	else
	{
		*pInt64 = m_listAudioOutput.front()->m_prepos[0];
	}

	pOut->m_bSeeked = true;
}

void CMP4SplitterFilter::DemuxSeek(REFERENCE_TIME rt,CBaseSplitterOutputPin* pOut,__int64* pInt64)
{
	CAutoLock lock(&m_csReadFile);
    AP4_Movie* movie = (AP4_Movie*)m_pFile->GetMovie();
    ...
    //while (pos) 
	{
        ...
        AP4_Track* track = movie->GetTrack(/*pPair->m_key*/pOut->m_nTrackNum);

        if (AP4_FAILED(track->GetSampleIndexForRefTime(rt, index))) 
		{
			ASSERT( 0 );
			*pInt64 = 0;
            return ;
        ...
        if (AP4_SUCCEEDED(track->GetSample(index, sample))) 
		{
			...
			{
				REFERENCE_TIME rtTmp = (REFERENCE_TIME)(10000000.0 / track->GetMediaTimeScale()*sample.GetCts());
				//m_rtStartOffset = rtTmp;
				m_mapStartOffset[pOut->m_nTrackNum] = rtTmp;
				char buf[260] = {0};
				sprintf_s(buf,259,"mp4 DemuxSeek rt %lld,rtTmp %lld\n",rt,rtTmp);
		...
		{
			//m_rtStartOffset = 0;
			m_mapStartOffset[pOut->m_nTrackNum] = 0;
			OutputDebugStringA("mp4 DemuxSeek Failed\n");
		}
...
HRESULT CMPEG4VideoSplitterFilter::CreateOutputs(IAsyncReader* pAsyncReader)
{
    ...
    CBaseSplitterOutputPin* pPinOut = DEBUG_NEW CBaseSplitterOutputPin(mts, L"Video", this, &hr,id,&mt);
...
{
	m_atpf = ((MPEG2VIDEOINFO*)((CBaseSplitterOutputPin*)CSource::GetPin(0))->CurrentMediaType().Format())->hdr.AvgTimePerFrame;

    return true;
}

void CMPEG4VideoSplitterFilter::DemuxOnlySeek(REFERENCE_TIME rt,CBaseSplitterOutputPin* pOut,__int64* pInt64)
{
	CAutoLock lock(&m_csReadFile);
}

void CMPEG4VideoSplitterFilter::DemuxSeek(REFERENCE_TIME rt,CBaseSplitterOutputPin* pOut,__int64* pInt64)
{
	CAutoLock lock(&m_csReadFile);
    ASSERT(rt == 0);
	m_rtMpeg4 = 0;
...
REFERENCE_TIME CMPEG4VideoSplitterFilter::getStartOffset(CBaseSplitterOutputPin* pOut)
{
	return 0LL;
}

bool CMPEG4VideoSplitterFilter::isEof()
{
	return false;
}

HRESULT CMPEG4VideoSplitterFilter::DemuxNextPacket(REFERENCE_TIME rtStartOffset,CBaseSplitterOutputPin* pOut,__int64* pInt64,Packet* pPacket)
{
	CAutoLock lock(&m_csReadFile);

	ASSERT(pPacket != NULL);
	pPacket->SetCount(0, 1024);
	pPacket->TrackNumber = 0;
	pPacket->rtStart = m_rtMpeg4;
	pPacket->rtStop = m_rtMpeg4 + m_atpf;
	pPacket->bSyncPoint = FALSE;
	m_rtMpeg4 += m_atpf;
...
//     CAutoPtr<Packet> pPacket;
    ...
    //REFERENCE_TIME rt = 0;
    //REFERENCE_TIME atpf = ((MPEG2VIDEOINFO*)pOut->CurrentMediaType().Format())->hdr.AvgTimePerFrame;
	...
	while(m_pFile->GetRemaining())
	{
		...
		{ // don't call CheckRequest so often
            bool bEof = !m_pFile->GetRemaining();

            if ( !pPacket->IsEmpty() && (m_pFile->BitRead(32, true) == 0x000001b6 || bEof)) 
			{
                return  DeliverPacket(pPacket,rtStartOffset,pOut);
            }
	...
	if ( !pPacket->IsEmpty() ) 
	{
		return  DeliverPacket(pPacket,0,pOut);
	}
	else
	{
		return S_FALSE;
...
CMP4SplitterOutputPin::CMP4SplitterOutputPin(CAtlArray<CMediaType>& mts, LPCWSTR pName, CSource* pFilter, HRESULT* phr,DWORD nTrackNum,CMediaType* pMT)
    : CBaseSplitterOutputPin(mts, pName, pFilter, phr,nTrackNum,pMT)
{
}
...
HRESULT CMP4SplitterOutputPin::dealPacket(Packet* p)
{
    CAutoLock cAutoLock(this);
    return __super::dealPacket(p);
}
