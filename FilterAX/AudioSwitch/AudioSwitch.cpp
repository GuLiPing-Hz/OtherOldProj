...
{
	//return pmt->majortype == MEDIATYPE_Audio ? S_OK:VFW_E_TYPE_NOT_ACCEPTED;
...
CMediaType CAudioSwitcherFilter::CreateNewOutputMediaType(CMediaType mt, long& cbBuffer)
{
	...
	GUID subtype = FOURCCMap(WAVE_FORMAT_PCM);
	if ( (mt.subtype != subtype) || (mt.formattype != FORMAT_WaveFormatEx) ||  (mt.cbFormat < sizeof(WAVEFORMATEX)))
	{
		return __super::CreateNewOutputMediaType(mt, cbBuffer);
	}
	WAVEFORMATEX* wfe = (WAVEFORMATEX*)pInPin->CurrentMediaType().pbFormat;
...
STDMETHODIMP CAudioSwitcherFilter::SwitchATrack(BOOL bFirstAudio)
{
	...
	{
		CAutoLock lock2(&m_csChangePin);
		m_bUsePin1 = bFirstAudio;
		if (m_bSwitchInit)
		{
			OutputDebugStringA("Use GLP Switch Audio\n");
			GetInputPin()->m_iGS->setNewPosition(/*m_rtStart +*/ m_rtStreamStart);
		}
		m_bChangePin = true;
		static bool b = true;
		if (b)
		{
			//m_bControlFirst = TRUE;
			b = false;
		}
	}
	...
	if (!m_bSwitchInit)
	{
		OutputDebugStringA("Use PRE Switch Audio\n");
		REFERENCE_TIME rtNewStart = 0;
		CComPtr<IMediaSeeking> pMS;
		if (m_pGraph)
		{
			HRESULT hr = m_pGraph->QueryInterface(IID_IMediaSeeking,(void**)&(pMS.p));
			if (SUCCEEDED(hr))
			{
				...
				{
					char buf[260] = {0};
					sprintf_s(buf,"rtNewStart %lld m_rtSampleDelta %lld\n",rtNewStart,m_rtSampleDelta);
					rtNewStart /*+= m_rtSampleDelta*/;
